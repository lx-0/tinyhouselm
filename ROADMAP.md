...
- `TINA-1215` — **Public `/arcs` index page + OG image. ✅** _(shipped 2026-04-27, depth #10.)_ Closes the public-index symmetry — every per-:slug surface (moment, character, zone, arc, digest) had a public page, but the *index* over named-pair relationship arcs was admin-only. Lists all named-pair arcs with pair glyph-doublets, current arc-label chip, affinity strength bar, freshest pair-moment headline, and a canonical `/arc/:slug` link per row. Optional `?rank=arc_strength|freshest` selector (default `arc_strength` desc). OG image via new `composeArcsIndexOg` reusing the TINA-616 pixel-art renderer (TINA · ARCS header, RELATIONSHIP ARCS title, single-row strip of up to 6 pair-doublets with first-name abbrevs, top arc label chip, freshest pair-moment headline). Separate disk-LRU OG cache (default 16, env-tunable via `ARCS_INDEX_OG_LRU_SIZE`) keyed on (top-pairs hash, freshest pair-moment id). Two new sticky-metrics counters (`arcsIndexViews`, `arcsIndexOgRenders`) deduped per (visitor-or-IP, UTC day). `aix` column in /admin sticky panel. Cross-links: /admin `named arcs` legend → `/arcs`; `/arc/:slug` footer → `/arcs`; `/moments` and `/characters` footers → `/arcs`. Per-IP rate limit (60/min, shared between page + image). No new persistence, no LLM, no sim hot-path.
