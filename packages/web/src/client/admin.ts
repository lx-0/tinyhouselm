...
interface StickyDailyRollup {
  ...
  /**
   * `/arcs` page hits, deduped per (visitor-or-IP) per day (TINA-1215).
   * Single global counter — the named-pair index has no per-filter buckets.
   */
  arcsIndexViews?: number;
  /**
   * `/arcs/og.png` renders, deduped per (visitor-or-IP) per day (TINA-1215).
   * Single global counter — the index OG has one cache key keyed on
   * (top-pairs hash, freshest pair-moment id).
   */
...
function renderStickyMetrics(payload: StickyMetricsPayload | null): void {
  ...
  thead.innerHTML =
    '<tr><th>date</th><th>shares</th><th>uniq</th><th>24h</th><th>7d</th><th>nudge</th><th>grp</th><th>aff</th><th>dig</th><th>zon</th><th>arc</th><th>cog</th><th title="moments-index OG renders, deduped per visitor-or-IP per day (TINA-1092)">iog</th><th title="characters-index page views, deduped per visitor-or-IP per day (TINA-1162)">cix</th><th title="arcs-index page views, deduped per visitor-or-IP per day (TINA-1215)">aix</th><th>rail</th><th title="rail CTR per variant: freshest / arc_strength (TINA-1020)">rctr</th></tr>';
  ...
  for (const d of rows) {
    ...
    const arcsIndex = d.arcsIndexViews ?? 0;
